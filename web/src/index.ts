...
// ─── Banco de dados (somente leitura – pode apontar para réplica) ─────────────
const pool = new Pool({
...
async function pollIrrigationLogs(): Promise<void> {
  try {
    const result = await pool.query(
      `SELECT sensor_id, zone, moisture, temperature, lamport_time, event_ts, worker_id, activated_at
         FROM irrigation_log
    ...
    // Banco pode não estar disponível ainda
  }
}

// ─── Bootstrap assíncrono (sem top-level await) ───────────────────────────────
async function bootstrap(): Promise<void> {
  ...
  app.get('/ws', { websocket: true }, (connection) => {
    // Em @fastify/websocket v10, o parâmetro já é a WebSocket diretamente
  ...
  app.post('/api/worker-status', async (req, reply) => {
    ...
    if (!body?.node_id) { return reply.status(400).send({ error: 'node_id obrigatório' }); }
  ...
  /** Retorna logs de irrigação recentes */
  app.get('/api/irrigation-logs', async (_req, reply) => {
    try {
      const result = await pool.query(
        'SELECT * FROM irrigation_log ORDER BY lamport_time DESC LIMIT 50'
      );
      return reply.send(result.rows);
    } catch {
      return reply.status(503).send({ error: 'Banco indisponível' });
    }
  });
  ...
  // ── Polling periódico ────────────────────────────────────────────────────────
  setInterval(pollIrrigationLogs, 3000);

  setInterval(() => {
    const now = Date.now();
    let changed = false;
    for (const [id, status] of workerStatuses) {
      if (now - status.updated_at > 15000 && status.status !== 'DOWN') {
  ...
  console.log(`[Web] Dashboard disponível em http://0.0.0.0:${PORT}`);
}
